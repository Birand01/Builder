...
public class WoodSpawner : MonoBehaviour
{
    ...
    {
        while (currentObjectAmount<maxObjectCapacity)
        {
            ...
            //.OnComplete(()=>
            // wood.transform.DOJump(new Vector3(woodSpawnPlaces[spawnIndex].position.x, woodSpawnPlaces[spawnIndex].position.y + yAxis,
            // woodSpawnPlaces[spawnIndex].position.z), 2f, 1, 0.5f).SetEase(Ease.OutQuad));
            spawnIndex++;
            currentObjectAmount++;
            if (spawnIndex >= 9)
